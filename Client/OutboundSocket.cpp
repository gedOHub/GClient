...
OutboundSocket::OutboundSocket(string ip, string port, int tag, fd_set* skaitomiSocket, fd_set* rasomiSocket, fd_set* klaidingiSocket):gNetSocket(ip, port, tag, skaitomiSocket, rasomiSocket, klaidingiSocket){
	this->write = true;
	this->name = "OutboundSocket";
	this->Connect();
}
void OutboundSocket::Connect(){
...
void OutboundSocket::Recive(SocketToObjectContainer^ container){
	using namespace std;
